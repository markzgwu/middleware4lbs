...
public final class Experiment12 {
	static int round = 10;
	...
	public static void experiment_attacker5(){
		...
		for(int i=0;i<1;i++){
			...
			for(int i1 = 0;i1<20;i1++){
				double memory_ability = 0.05*i1;
				double[] far_frr = experiment_attacker(memory_ability);
				far+=far_frr[0]+",";
				frr+=far_frr[1]+",";
				mem_ability+=memory_ability+",";
			}
			
			output+=("memory_ability:"+mem_ability)+"\n";
			output+=("FAR:"+far)+"\n";
			output+=("FRR:"+frr)+"\n";			
		}
	...
	public static double[] experiment_attacker(double memory_ability){
		...
		parameters.total_request = 2;
		parameters.movies_len = 20;
		parameters.memory_ability = memory_ability;
		...
		for(int userid=1;userid<100;userid++){
			...
			mydp.setInput_evdience(ag.attacker_determined());
